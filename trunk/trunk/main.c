...
mode_t orig_umask = 0;
int batch_gen_fd = -1;
uint32 p1 = DEFAULT_P1;
uint32 p2 = DEFAULT_P2;
...
static void do_server_recv(int f_in, int f_out, int argc, char *argv[])
{
    ...
    // Here we can receive p1 and p2 generated by client
    if (use_random) {
        int recv_random_data_status;
        recv_random_data_status = recv_random_data(f_in);
        if (recv_random_data_status < 0) {
            rprintf(FERROR,"server_recv: recv_random_data error\n");
		    exit_cleanup(RERR_RANDOM);
        }
    }
...
int client_run(int f_in, int f_out, pid_t pid, int argc, char *argv[])
{
	...
	if (am_sender) {
        ...
        // Here we can add option to send p1 and p2
        if (use_random) {
            int send_random_data_status;
            send_random_data_status = send_random_data(f_out);
            if (send_random_data_status < 0) {
                rprintf(FERROR,"client_run: send_random_data error\n");
		        exit_cleanup(RERR_RANDOM);
            }
        }
		flist = send_file_list(f_out, argc, argv);
