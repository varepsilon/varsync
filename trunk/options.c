...
void usage(enum logcode F)
{
  ...
  rprintf(F,"     --irreducible_poly      use hashing by irreducible polinomial division for rolling sum\n");
...
enum {OPT_VERSION = 1000, OPT_DAEMON, OPT_SENDER, OPT_EXCLUDE, OPT_EXCLUDE_FROM,
      ...
      OPT_IRREDUCIBLE_POLY, OPT_SMALL_BLENGTH,
...
static struct poptOption long_options[] = {
  ...
  {"irreducible_poly", 0,  POPT_ARG_NONE,   0, OPT_IRREDUCIBLE_POLY, 0, 0 },
...
int parse_arguments(int *argc_p, const char ***argv_p)
{
	...
	while ((opt = poptGetNextOpt(pc)) != -1) {
        ...
        case OPT_IRREDUCIBLE_POLY:
            use_irreducible_poly = 1;
            break;
